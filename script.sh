...
cat domains-test.txt | grep -Po "(\w+\,\w+\,\w+)$" | sort -u >>third-level.txt

echo "Gathering full third-level domains with Sublist3r.."
for domain i $(cat third-level.txt); do sublist3r -d domain -o thirdlevels/$domain.txt; cat thirdlevels/$domain.txt | sort -u >> final.txt; done
